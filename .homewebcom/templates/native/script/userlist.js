...
function setBirthday(nick, birthday) {
    ...
    if (birthday == "1" && !bdayEl) {
        ...
        bdayEl.title = "Hat heute Geburtstag";
        bdayEl.textContent = "\uD83C\uDF82";
...
function getUserGroupLabel(group) {
    switch (group) {
        ...
        case "mod":        return "Superuser";
        case "registered": return "Chatter";
...
function buildUserMenu(un, st, ga) {
    ...
    if (isReg) {
        ...
        menu += `<a href="javascript:showChatPanel('${profileUrl}','Profil: ${un}');"><span class="icon">\uD83D\uDC64</span>Profil anzeigen</a>`;
    }

    if (!isSelf) {
        menu += `<a href="#" data-act="query"><span class="icon">\uD83D\uDCAC</span>Privatchat starten</a>`;
    }

    if (own >= 3 && !isSelf) {
        menu += `<div class="divider"></div><div class="group-label">Moderation</div>`;
        if (st == 1) {
            menu += `<a href="#" data-act="su"><span class="icon">\u2B50</span>Superuser-Rechte vergeben</a>`;
        }
        if (st == 3) {
            menu += `<a href="#" data-act="rsu"><span class="icon">\uD83D\uDEAB</span>Superuser-Rechte nehmen</a>`;
        }
        if (own >= st) {
            menu += `<a href="#" data-act="k"><span class="icon">\uD83D\uDC62</span>Chatter wegkicken</a>`;
            if (ga == 1) {
                menu += `<a href="#" data-act="ungag"><span class="icon">\uD83D\uDD0A</span>Chatter entknebeln</a>`;
            } else {
                menu += `<a href="#" data-act="gag"><span class="icon">\uD83D\uDD07</span>Chatter knebeln</a>`;
            }
        }
    }
    if (own >= 5 && !isSelf) {
        menu += `<div class="divider"></div><div class="group-label">Rauswurf</div>`;
        menu += `<a href="#" data-act="hk" class="danger"><span class="icon">\uD83D\uDEAA</span>Chatter rauswerfen</a>`;
        menu += `<a href="#" data-act="bantemp" class="danger"><span class="icon">\u23F1</span>1 Stunde verbannen</a>`;
    }
    if (own >= 9 && !isSelf) {
        menu += `<a href="#" data-act="ban" class="danger"><span class="icon">\u26D4</span>Dauerhaft verbannen</a>`;
    }
    return menu;
}
...
function buildUserItem(u) {
    ...
    const awayTitle = as == 1 ? "Abgemeldet: " + escapeHtml(sr || "") : "Aktionen fuer " + escapeHtml(un);
    const bdayHtml  = birthday == 1
        ? `<span class="user-list-birthday" title="Hat heute Geburtstag">\uD83C\uDF82</span>`
        : "";
    ...
    return `
        <div class="${itemClass}" data-nick="${escapeHtml(un)}" data-role="${role}">
            ...
            <button class="user-list-toggle" type="button" aria-label="Aktionen fuer ${escapeHtml(un)}" aria-expanded="false">\u22EE</button>
    ...
    host.innerHTML = `
        <div class="user-list-container" id="user-list-container">
            ...
            <div class="user-list-search" id="user-list-search">
                <input type="text" id="user-list-filter" placeholder="Nicks filtern..." autocomplete="off" spellcheck="false">
                <button type="button" class="clear-btn" id="user-list-filter-clear" aria-label="Filter loeschen">\u00D7</button>
            </div>
            <div class="user-list-scroll" id="user-list-scroll">
                <div class="user-list-empty" id="user-list-empty">
                    <span class="emoji">\uD83D\uDC4B</span>
                    Noch keine Chatter im Raum
                </div>
            ...
            <div class="user-list-counter" id="user-list-counter">0 Chatter</div>
        </div>
    `;
...
function updateCounter() {
    ...
    if (el) {
        const n = user.length;
        el.textContent = n + (n === 1 ? " Chatter" : " Chatter");
    }
}
